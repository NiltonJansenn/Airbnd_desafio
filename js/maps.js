...
let input;
let currentLoc;
let Belo_horizonte= { lat: -19.8157, lng: -43.9542 };
...
function pesquisar_Locations() {
  input = document.getElementById("location-search").value;
  let geocoder = new google.maps.Geocoder();
  geocoder.geocode({ address: input }, function (results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
      pesquisar_LocationsNear(results[0].geometry.location);
    } else {
...
function pesquisar_LocationsNear() {
